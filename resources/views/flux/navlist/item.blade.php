...
$classes = Flux::classes()
    ...
    ->add(match ($variant) {
        'outline' => match ($accent) {
            true => [
                'data-current:text-(--color-yellow-400) hover:data-current:text-(--color-yellow-500)',
                'data-current:bg-yellow-400/[30%] dark:data-current:bg-white/[7%] data-current:border data-current:border-zinc-200/[0%] dark:data-current:border-transparent',
                'hover:text-yellow-50 dark:hover:text-white dark:hover:bg-white/[7%] hover:bg-zinc-800/5 ',
...
<flux:button-or-link :attributes="$attributes->class($classes)" data-flux-navlist-item>
    ...
    <?php if ($slot->isNotEmpty()): ?>
        <div class="flex-1 text-sm font-medium leading-none whitespace-nowrap [[data-nav-footer]&]:hidden [[data-nav-sidebar][data-nav-footer]_&]:block" data-content>{{ $slot }}</div>
    <?php endif; ?>
    ...
    <?php endif; ?>
</flux:button-or-link>    @php $iconTrailing = $iconTrailing ??= $attributes->pluck('icon:trailing'); @endphp
    @php $iconVariant = $iconVariant ??= $attributes->pluck('icon:variant'); @endphp
